...
to setup ;; Para inicializar la simulación.
  ...
  ;; Para inicializar TODAS las parcelas desde un archivo de datos reales.
    S-P-init
    
  ask patches ;; Para inicializar cada una con su color y used? en false
  [
    P-init
  ]

  
  ;; Para crear tortugas e inicializarlas
  crt 50
  [
    T-init
  ]
...
to go  ;; Versión con el graficador de q contra tics.
  ask turtles [T-move]
  ;; plot corridor-width
  tick
  if ticks >= 1000 
  [
    let parcelasOcupadas cantidadParcelasConMariposas
    show parcelasOcupadas
    plotxy q parcelasOcupadas
    stop
  ]
end
...
to-report cantidadParcelasConMariposas
  report count patches with [(count turtles-here) >= 1]
end
;;*******************************
...
to T-init ;; Para inicializar una tortuga a la vez.
  set size 2
  let x random (maxX - minX) + minX
  let y random (maxY - minY) + minY
  setxy x y
...
to S-P-init ;; Para inicializar la variable elevation de todas las parcelas con datos de un archivo.
...
to P-init ;; Para inicializar used? y color de una parcela a la vez
  set pcolor scale-color green elevation 400 700
  set used? false
end
;;****************************
