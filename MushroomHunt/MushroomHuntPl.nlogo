...
to setup ;; Para inicializar la simulación.
  ca ;; restablece todas las variables de ambiente para iniciar simulación nueva.
  init-globals
  ;; Deberían crearse e inicializarse tortugas y parcelas.
  P-init ;; inicializa las parcelas
  crt 2
  [
    T-init
  ]  
...
to T-init ;; Para inicializar una tortuga a la vez.
  set size 2
  set color yellow
  set timeSinceLastFound 999
end

;; Se debería cambiar el nombre para que represente algo signficativo en la simulación.
to T-search
  ifelse timeSinceLastFound <= 20
     [right (random 181) - 90]
     [right (random 21) - 10]
  forward 1
  ifelse pcolor = red
    [
      set timeSinceLastFound 0
      set pcolor yellow
    ]
    [
      set timeSinceLastFound timeSinceLastFound + 1
    ]
end
;;*******************************
